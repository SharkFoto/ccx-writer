...
走同一套门禁,CI 里跑)之后 **24 个杀掉 23 个**。剩下的 `_add_pen` 封顶 32767 改成 65535 是**等价变异**:
量程已经把笔宽算进去,笔宽单位到不了 32512 以上,封顶那一支不可达。
...
- BeginPage / BeginGroup 的包围盒按 UC2 读取侧的布局写成 4 x s32,libcdr 16 位按 4 x s16 读。两份实现都是照真实
  CorelDRAW 文件写的,互相矛盾;没有 CorelDRAW 自己导出的 16 位 CMX 样本之前不动字段宽度(值已按 #29 修正)
