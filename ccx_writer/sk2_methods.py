...
# 原 uc2/formats/sk2/sk2_methods.py,逐行对应移植。
#
# 与原版的差异只有导入:
#   from uc2 import _, uc2const, libgeom    -> 下面三行(_ 来自 ccx_writer.translator)
#   from uc2.formats.sk2 import sk2_model   -> from ccx_writer import sk2_model
#
# Py2 -> Py3 静默差异逐项核过,本文件没有需要改写的:
#   - 没有 round / 整数除法语境:fit_page_to_image / fit_to_page 里的 / 两侧
#     都是 bbox 或页面尺寸的浮点,保持 /(不是 //)。
#   - ' %i' % n 与 _('Layer') / _('Page') 的拼接在 Py3 产出同样的字符串
#     ('Layer 1'、'Page 2'),translator._ 与原版 dummy_translate 一样是恒等。
#   - 没有 dict 遍历、map/filter/zip、basestring/unicode、cmp 排序。
#   - class SK2_Methods: 在 Py2 是旧式类、Py3 是新式类;这里只有普通方法和
#     一个 @staticmethod(旧式类里同样生效),没有可观察的差别,保持原样。

from copy import deepcopy

from ccx_writer import uc2const, libgeom
