...
# 移植说明:逐行照搬原版 uc2/formats/svg/svg_colors.py,颜色名表,零改动。
# 浮点字面量在 Py2.7/Py3 上解析结果逐位一致。
# 表内原样保留的上游瑕疵(会进入 sk2 颜色的第 4 项「颜色名」,不可修):
#   'peru' 的名字写成了 'per';'aqua' 的名字是 'cyan' 而 'fuchsia' 仍叫
#   'fuchsia';*grey 的名字统一写成 *gray。
