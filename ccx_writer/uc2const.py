...
# 移植说明:逐行照搬原版 uc2/uc2const.py,纯常量表。
# 唯一改动是下面这行导入(原为 from uc2 import _)。
# 浮点字面量与浮点运算(in_to_pt / 2.54 等)在 Py2.7 与 Py3 上是同一套
# IEEE 754 双精度 + 正确舍入解析,数值逐位一致,无需任何处理。
# 所有表都只被按键取值(FORMAT_NAMES[cid]、PAGE_FORMATS[fmt]),前端
# 没有遍历这些 dict 的地方,Py2 哈希序与 Py3 插入序的差异不影响输出。
