...
#
#  移植说明(uc2/formats/sk2/sk2_model.py):
#  - 逐行对应原版;函数名、类名、变量名、语句顺序不变,原版的 bug 一律原样保留
#    (输出要与 Py2 逐位一致),发现的问题只记录不修。
#  - 范围裁剪:Text 依赖 libpango 的方法(get_glyphs / update / to_curve /
#    get_transformed_paths)抛 NotImplementedError;Pixmap(ImageMagick)构造即抛。
#  - arrows(uc2/formats/sk2/arrows.py)不在模块映射内,见下方 _ArrowsStub。
...
from ccx_writer.sk2_cids import *
from ccx_writer.translator import _
from ccx_writer import cms, uc2const, libgeom, sk2const
from ccx_writer.generic import TextModelObject
from ccx_writer._compat import py2round
# 移植版不要:EditableImageHandler(uc2.libimg.handlers,ImageMagick 位图,范围外)
...
_TEXT_NOT_SUPPORTED = ('文字需在上游先转曲线(如 inkscape --export-text-to-path);'
                       'ccx_writer 不带 pango')
_PIXMAP_NOT_SUPPORTED = '位图不支持:CMX v1 写出器不写位图'


class _ArrowsStub(object):
    """
    原版 `from . import arrows`。arrows 模块不在移植的模块映射内。
    只有 stroke[9](markers)非空时 Curve.update_arrows 才会调它;
    SVG 前端建的描边 markers 恒为 [](svg_translators 写死),不可达。
    真需要时把 uc2/formats/sk2/arrows.py 移植成 ccx_writer.arrows,
    并把下面 `arrows = _ArrowsStub()` 换成 `from ccx_writer import arrows`。
    """

    def get_arrow_cpath(self, arrow, trafo=None):
        raise NotImplementedError('箭头不支持:ccx_writer 未移植 sk2 arrows'
                                  '(SVG 前端不产生 stroke markers)')

    def get_arrow_paths(self, arrow):
        raise NotImplementedError('箭头不支持:ccx_writer 未移植 sk2 arrows'
                                  '(SVG 前端不产生 stroke markers)')
...
class DocumentObject(TextModelObject):
    ...
    def copy(self, src=None, dst=None):
        ...
        props = self.__dict__
        # Py3:keys() 是视图;这里遍历 props、写的是 obj_copy.__dict__,不改被遍历的 dict
...
class Layer(StructuralObject):
    ...
    def update(self):
        # Py2 这里是 str(字节串),unicode 颜色串不会被转换;Py3 只有 str,
        # 默认配置(layer_color='#3252A2')两边都走转换分支
...
class TP_Group(Group):
    ...
    def set_text_on_path(self, path_obj, text_obj, data):
        # 原版调 libgeom.set_text_on_path(libgeom.text_on_path,路径排字,
        # 依赖文字字形 = libpango),移植版 libgeom 不带
...
class Rectangle(PrimitiveObject):
    ...
    # 注意:默认参数在 def 时求值一次,所有实例共享同一个列表对象(原版如此,保留)
    def __init__(self, config, parent=None,
    ...
    def update_arrows(self):
        ...
        if self.is_curve and self.style[1]:
            ...
            for path in self.paths:
                ...
                # end arrow
                # Py2 的 isinstance(x, int) 不含 long;Py3 int 即全部整数
...
class Text(PrimitiveObject):
    ...
    移植版范围裁剪:字形布局 / 转曲线依赖 libpango,不支持(见各方法)。
    """
    ...
    def __init__(self, config, parent=None,
        ...
        self.config = config
        self.parent = parent
        # Py3:b64encode 只收 bytes。Py2 这里收的是字节串 str,等价于先按 utf-8 编码
    ...
    def set_text(self, text):
        # Py2: isinstance(text, unicode) -> Py3: str
    ...
    def get_glyphs(self):
        # 移植版不要:原体调 libgeom.get_text_glyphs(libpango 排版出字形 cpath)
        raise NotImplementedError(_TEXT_NOT_SUPPORTED)
    ...
    def to_curve(self):
        # 移植版不要:原体逐字形 libgeom.get_paths_from_glyph 拼 Group(依赖 pango 字形)
        raise NotImplementedError(_TEXT_NOT_SUPPORTED)

    def get_transformed_paths(self):
        # 移植版不要:原体调 libgeom.get_paths_from_glyph(依赖 pango 字形)
        raise NotImplementedError(_TEXT_NOT_SUPPORTED)

    def update(self):
        # 移植版不要:原体 self.cache_cpath = self.get_glyphs() 后逐字形 apply_trafo、
        # update_bbox、style[0][0] = FILL_NONZERO —— 第一步就依赖 libpango
        raise NotImplementedError(_TEXT_NOT_SUPPORTED)
...
class Pixmap(PrimitiveObject):
    ...
    移植版范围裁剪:位图依赖 EditableImageHandler(ImageMagick),构造即抛。
    其余方法照抄留作对照,构造不出实例所以不可达。
    """
    ...
    def __init__(self, config, parent=None,
                 ...
                 trafo=[] + sk2const.NORMAL_TRAFO,
                 style=[] + sk2const.EMPTY_STYLE):
        # 移植版不要:原体 self.handler = EditableImageHandler(self) 等(ImageMagick)
        raise NotImplementedError(_PIXMAP_NOT_SUPPORTED)
    ...
    def get_resolution(self):
        ...
        # Py2 round -> py2round(四舍五入,非银行家舍入)
        v_dpi = int(py2round(uc2const.in_to_pt / m11))
        h_dpi = int(py2round(uc2const.in_to_pt / m22))
        return h_dpi, v_dpi
